...
export function tmagicMaterialFiles(theme?: TenantPlan['theme']): Record<string, string> {
  ...
  <div class="tmagic-page-runtime min-h-screen bg-default text-default font-sans antialiased flex flex-col justify-between selection:bg-primary-500 selection:text-white">
...
`,
  ...
  >
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      ...
      <!-- 3. featureGrid 模式 (支持 bordered 与默认 cards 变体) -->
...
`,
  ...
  <footer class="bg-neutral-900 text-neutral-400 py-12 text-sm border-t border-neutral-800">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div>
          <div v-for="(ln, i) in node.lines || []" :key="i" class="text-neutral-300 font-medium mb-1.5">
            {{ ln }}
          </div>
        </div>
        <div v-if="node.contact" class="space-y-1 text-xs text-neutral-400">
      ...
      <div class="pt-6 border-t border-neutral-800 text-xs text-neutral-500 flex flex-col sm:flex-row justify-between items-center gap-2">
