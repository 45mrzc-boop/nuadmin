...
/**
 * Returns the first primary shade that achieves target contrast (defaults to 4.5 for WCAG AA).
 * For light backgrounds (like white), searches from 500 down to 950.
 * For dark backgrounds, searches from 400 up to 50.
 */
export function fgOn(surface: string, shades: string[], target = 4.5): { shadeName: string, hex: string, ratio: number } {
  const names = ['50', '100', '200', '300', '400', '500', '600', '700', '800', '900', '950']
  const surfLum = relativeLuminance(surface)
  const isLightSurface = surfLum > 0.4
  if (isLightSurface) {
    for (let i = 5; i < shades.length; i++) {
      const ratio = contrastRatio(shades[i], surface)
      if (ratio >= target) return { shadeName: names[i], hex: shades[i], ratio }
    }
    return { shadeName: '950', hex: shades[10], ratio: contrastRatio(shades[10], surface) }
  } else {
    for (let i = 4; i >= 0; i--) {
      const ratio = contrastRatio(shades[i], surface)
      if (ratio >= target) return { shadeName: names[i], hex: shades[i], ratio }
    }
    return { shadeName: '50', hex: shades[0], ratio: contrastRatio(shades[0], surface) }
  }
}
...
${shades.map((s, i) => `  --color-primary-${names[i]}: ${s};`).join('\n')}
  ...
  --color-primary-fg-light: var(--color-primary-${lightFg.shadeName});
  --color-primary-fg-dark: var(--color-primary-${darkFg.shadeName});
  --color-primary-fg-badge: var(--color-primary-${badgeFg.shadeName});
...
:root {
  ...
  --ui-primary-fg-light: var(--color-primary-${lightFg.shadeName});
