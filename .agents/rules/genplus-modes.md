...
## 5. 端口规范与主从分配指引 (Port Convention & Registry)

- **真理来源**：`<repoRoot>/main-admin/.env` 中的 `PORT` 与 `TENANT_PORT_FROM`；
- **默认端口规划**：
  * **主控制台 (Main Admin)**：默认运行于 `http://localhost:10000`；
  * **子后台 (Sub Admin)**：默认从 `PORT + 1`（即 `10001`）起，按创建顺序自动递增分配空闲端口（`10001`, `10002`, `10003`...）；
- **人机交互与自主指定指引**：
  * AI 在立项对齐时，必须在方案概要中明确展示默认端口，并提示用户可自主指定（如“默认分配端口为 `http://localhost:10001`，如需自定义端口可直接说明”）；
  * 用户若指定端口（如“端口用 10086”），调用 MCP 工具 `genplus_create_tenant` 时必须显式传入 `port: 10086` 参数；
  * 系统提供端口范围（1024~65535）、主控制台防冲突、租户库防重与宿主机动态探活双重门禁保障。
